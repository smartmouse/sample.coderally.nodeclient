...
exports.recalculateHeading = function (car, bias) {
	...
	var vel_magnitude = math.sqrt(math.pow(vel.x) + math.pow(vel.y));
	var accel = car.getCarStatus().getAcceleration();
	var accel_magnitude = math.sqrt(math.pow(accel.x) + math.pow(accel.y));
...
function getIntersectionPointWithThreshold(checkpoint, rotation, point, threshold) {
	var rotation_in_degrees = math.unit(rotation, 'deg');
	var rotation_in_radians = rotation_in_radians.toNumber('rad');
